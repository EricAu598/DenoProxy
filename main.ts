...
// 处理 reverseEric 路径的函数，过滤出纯文本
async function handleReverseEric(req: Request): Promise<Response> {
    try {
        // 确保请求有内容
        if (!req.body) {
        ...
        console.error("处理 reverseEric 请求时发生错误:", err);
        return new Response(`Error processing reverseEric request: ${err}`, { status: 500 });
    }
}
...
await serve(async (req) => {
    ...
    // 处理新的 /proxy/reverseEric 路径
    if (url.pathname === "/proxy/reverseEric") {
        return handleReverseEric(req);
    }
    ...
    return new Response(
        ...
        "3. 使用 /proxy/reverseEric 过滤出纯文本内容。\n" +
